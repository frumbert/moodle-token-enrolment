...
class login_signup_form extends moodleform
{
	...
	function definition() {
		...
		$mform->addElement('text', 'token', get_string('signup_tokencode', 'auth_token'), array("maxlength" => 12, "size" =>15)); //, "value" => $tokenqs));
		$mform->setType('token', PARAM_NOTAGS);
		$mform->setDefault('token', $tokenqs);
